...
class SC_weighted_BERT(BertPreTrainedModel):
    def __init__(self, config,weights):
        ...
        self.loss_fct = torch.nn.CrossEntropyLoss(weight=torch.tensor(weights, dtype=torch.float))
        self.init_weights()
    ...
    def forward(
        ...
        if labels is not None:
            loss = self.loss_fct(logits.view(-1, self.num_labels).float(), labels.view(-1).long())
...
import sys
trainData =  pd.read_csv(sys.argv[1], sep='\t', names=['text', 'label'], header=None, encoding='utf-8', error_bad_lines=False)
valData =  pd.read_csv(sys.argv[2], sep='\t', names=['text', 'label'], header=None, encoding='utf-8', error_bad_lines=False)
testData =  pd.read_csv(sys.argv[3], sep='\t', names=['text', 'label'], header=None,encoding='utf-8', error_bad_lines=False)
...
print(trainData)
print("IS NAN train") 
print(trainData['label'].isna().any())
print("IS NAN valData") 
print(valData['label'].isna().any())
print("IS NAN testData")
print(testData['label'].isna().any())
print("IS NAN train") 
print(trainData['text'].isna().any())
print("IS NAN valData")
print(trainData['text'].isna().any())
print("IS NAN testData")
print(trainData['text'].isna().any())
...
X_train, attention_masks_train, y_train = getFeaturesandLabel(X_tn,le.fit_transform(y_tn))
X_val, attention_masks_val, y_val = getFeaturesandLabel(X_v,le.fit_transform(y_v))
X_test, attention_masks_test, y_test = getFeaturesandLabel(X_ts,le.fit_transform(y_ts))
